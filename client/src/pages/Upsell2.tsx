...
export default function Upsell2() {
  ...
  const handleAccept = async () => {
    setLoading(true);
    track("upsell_accept", { page: "upsell2", value: { chronotype, price: 17 } });
    try {
      const result = await orderMutation.mutateAsync({ sessionId: getSessionId(), productId: "oto2", chronotype });
      window.location.href = result.gumroadUrl;
    } catch {
      window.location.href = "https://deepsleepreset.gumroad.com/l/cuhln";
    }
  };
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: "oklch(0.07 0.025 255)" }}>
      ...
      <CountdownTimer variant="banner" label="Exclusive upgrade expires in:" />
      <div className="relative z-10 container max-w-lg mx-auto py-10">
        <div className="text-center mb-8">
          <div className="badge-popular mb-3">Exclusive Upgrade</div>
          <div className="text-5xl mb-4">{icon}</div>
          <h1 className="font-display font-bold text-2xl md:text-3xl mb-3" style={{ color: "oklch(0.95 0.01 265)" }}>
            The {chronotype} Deep Sleep Mastery Course
          </h1>
          <p className="text-sm" style={{ color: "oklch(0.60 0.04 265)" }}>
            Go beyond the 7-night protocol with a complete video course built for <strong style={{ color: "oklch(0.82 0.16 65)" }}>{chronotype}</strong> chronotypes.
          </p>
        </div>
        <div className="glass-card rounded-3xl p-8 mb-6" style={{ border: "1px solid oklch(0.78 0.18 65 / 0.3)" }}>
          <h2 className="font-bold text-lg mb-4" style={{ color: "oklch(0.82 0.16 65)" }}>
            🎓 {chronotype} Mastery Course (6 modules)
          </h2>
          <div className="flex flex-col gap-2 mb-6">
            {[
              `Module 1: Understanding your ${chronotype} biology`,
              "Module 2: Building your ideal sleep environment",
              `Module 3: ${chronotype}-specific nutrition for deep sleep`,
              "Module 4: Stress & cortisol management protocol",
              "Module 5: Advanced sleep tracking & optimization",
              "Module 6: Long-term maintenance & troubleshooting",
            ].map((item, i) => (
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="text-base line-through" style={{ color: "oklch(0.40 0.04 265)" }}>$97</span>
            <span className="font-black text-4xl" style={{ color: "oklch(0.82 0.16 65)" }}>$17</span>
            <div className="badge-popular">82% OFF</div>
          </div>
          <CountdownTimer variant="inline" label="Upgrade offer expires in:" />
          <button onClick={handleAccept} disabled={loading}
            ...
            <span>{loading ? "Processing..." : `Yes! Upgrade to Mastery Course — $17`}</span>
        ...
        <button onClick={handleDecline}
          ...
          <X className="w-3 h-3" />
          No thanks, I'll skip the Mastery Course
        </button>
      </div>
    </div>
  );
