...
export async function sendPurchaseConfirmation({
  ...
  const downloadsToShow: Array<{ name: string; url: string }> = [PRODUCT_DOWNLOADS.tripwire];
  if (product !== "tripwire" && PRODUCT_DOWNLOADS[product as keyof typeof PRODUCT_DOWNLOADS]) {
    downloadsToShow.push(PRODUCT_DOWNLOADS[product as keyof typeof PRODUCT_DOWNLOADS]);
  }
  ...
  const downloadItemsHtml = downloadsToShow.map(dl => `
    <div style="background:#0d0b1a;border:1px solid #2d2050;border-radius:10px;padding:16px;margin-bottom:12px;">
      <p style="color:#c4a8ff;font-weight:700;margin:0 0 4px;font-size:14px;">📄 ${dl.name}</p>
      <p style="color:#6b5f8a;margin:0 0 12px;font-size:12px;">Personalized for ${chronotypeName}</p>
      <a href="${dl.url}" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#5b21b6);color:#fff;text-decoration:none;padding:10px 20px;border-radius:8px;font-size:13px;font-weight:700;">
        ⬇️ Download PDF
      </a>
    </div>`).join("");
...
<head>
  ...
  <title>Your materials are ready — Deep Sleep Reset</title>
</head>
<body style="margin:0;padding:0;background:#0d0b1a;font-family:Inter,Arial,sans-serif;color:#e8e6f0;">
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <!-- Success banner -->
    <div style="background:linear-gradient(135deg,#2d1b69,#1a0f3a);border:1px solid #4a3080;border-radius:16px;padding:28px;text-align:center;margin-bottom:24px;">
      <div style="font-size:40px;margin-bottom:10px;">✅</div>
      <h2 style="color:#fff;margin:0 0 8px;font-size:22px;font-weight:800;">Payment confirmed!</h2>
      <p style="color:#a89bc8;margin:0;font-size:14px;">Order processed · $${amount.toFixed(2)} USD · Instant access below</p>
    </div>

    <!-- Chronotype badge -->
    <div style="background:#1a1230;border:1px solid #2d2050;border-radius:12px;padding:16px;text-align:center;margin-bottom:24px;">
      <p style="color:#8b7fa8;margin:0 0 4px;font-size:11px;text-transform:uppercase;letter-spacing:2px;">Your chronotype</p>
      <p style="color:#c4a8ff;font-size:26px;font-weight:800;margin:0;">${chronotypeName}</p>
      <p style="color:#6b5f8a;font-size:12px;margin:4px 0 0;">Your protocols are personalized for this type</p>
    </div>

    <!-- Download section -->
    <div style="background:#1a1230;border:1px solid #2d2050;border-radius:16px;padding:24px;margin-bottom:24px;">
      <h3 style="color:#fff;margin:0 0 16px;font-size:17px;">📥 Your downloads — click to access now</h3>
      ${downloadItemsHtml}
      <!-- Bonus -->
      <div style="background:#0d0b1a;border:1px solid #c4a8ff33;border-radius:10px;padding:14px;display:flex;align-items:center;justify-content:space-between;">
        <div>
          <p style="color:#c4a8ff;font-weight:700;margin:0 0 2px;font-size:14px;">📊 Sleep Score Tracker</p>
          <p style="color:#6b5f8a;margin:0;font-size:12px;">Bonus — included in your guide</p>
        </div>
        <span style="background:#c4a8ff22;color:#c4a8ff;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;border:1px solid #c4a8ff44;">BONUS</span>
      </div>
    </div>
    ...
    <div style="background:linear-gradient(135deg,#1a0f3a,#0d0b1a);border:1px solid #4a3080;border-radius:16px;padding:24px;margin-bottom:24px;">
      <h3 style="color:#fff;margin:0 0 14px;font-size:16px;">🚀 Start tonight (5 minutes)</h3>
      <ol style="color:#a89bc8;margin:0;padding-left:20px;font-size:14px;line-height:2;">
        <li>Download your personalized plan above</li>
        <li>Read the Night 1 protocol (5 minutes)</li>
        <li>Set your alarm for the time recommended for ${chronotypeName}</li>
        <li>Track your Sleep Score every morning</li>
      </ol>
    </div>

    <!-- Upsell CTA -->
    <div style="background:linear-gradient(135deg,#2d1b0a,#1a0f3a);border:2px solid #c4a83366;border-radius:16px;padding:24px;margin-bottom:24px;text-align:center;">
      <p style="color:#c4a833;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:2px;margin:0 0 8px;">🔥 One-Time Offer — 15 Minutes Only</p>
      <h3 style="color:#fff;margin:0 0 8px;font-size:18px;font-weight:800;">Upgrade to the Elite Bundle</h3>
      <p style="color:#a89bc8;margin:0 0 6px;font-size:13px;">All 4 protocols + toolkit + lifetime updates</p>
      <p style="color:#6b5f8a;font-size:13px;margin:0 0 16px;">Value: <span style="text-decoration:line-through;">$297</span> → <strong style="color:#c4a833;font-size:20px;">$47</strong> one-time</p>
      <a href="${baseUrl}/thank-you" style="display:inline-block;background:linear-gradient(135deg,#c4a833,#e8c84a);color:#0d0b1a;text-decoration:none;padding:14px 32px;border-radius:10px;font-size:15px;font-weight:800;">
        Upgrade Now — $47 →
      </a>
      <p style="color:#4a3f6b;font-size:11px;margin:10px 0 0;">30-day money-back guarantee · Instant access</p>
    </div>
  ...
  return brevoRequest("/smtp/email", {
    ...
    subject: `✅ Your materials are ready — ${purchasedDownload.name} + Download Links`,
    htmlContent,
  });
}
  ...
  },
  {
    ...
    content: (_chronotype: string) => `
      ...
      <div style="background:#1a1230;border:1px solid #2d2050;border-radius:12px;padding:16px;margin:16px 0;text-align:center;">
        ...
        <a href="https://deep-sleep-reset.com/upsell/1" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#5b21b6);color:#fff;text-decoration:none;padding:10px 24px;border-radius:8px;font-size:13px;font-weight:700;">
          View Program →
        </a>
      </div>
    `,
  },
  {
    ...
    content: (chronotype: string) => `
      ...
      <div style="background:linear-gradient(135deg,#2d1b69,#1a0f3a);border:1px solid #4a3080;border-radius:16px;padding:24px;margin:16px 0;text-align:center;">
        ...
        <a href="https://deep-sleep-reset.com/upsell/1" style="display:inline-block;background:linear-gradient(135deg,#7c3aed,#5b21b6);color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-size:14px;font-weight:700;">
          Continue My Journey →
        </a>
      </div>
